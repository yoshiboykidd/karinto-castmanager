...
export default function MyPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FFFDFE] pb-36 font-sans text-gray-800 overflow-x-hidden">
      <CastHeader shopName="マイページ" displayName={profile?.display_name} version="v4.5.0" bgColor={currentTheme.bg} />
      <main className="px-5 mt-6 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="space-y-6">
          <section className="bg-white border border-pink-50 rounded-[40px] p-8 shadow-xl shadow-pink-100/20 space-y-6">
            <h3 className="font-black text-gray-700">🎨 テーマカラー</h3>
          ...
          <section className="bg-white border border-pink-50 rounded-[40px] p-8 shadow-xl shadow-pink-100/20 space-y-6">
            <h3 className="font-black text-gray-700">💰 今月の目標金額</h3>
            <div className="relative">
              <input type="text" inputMode="numeric" value={targetAmount} onChange={(e) => setTargetAmount(e.target.value)} className="w-full px-6 py-5 pl-12 rounded-3xl bg-gray-50 border-none font-black text-2xl text-gray-700 focus:ring-4 focus:ring-pink-100" />
              <span className="absolute left-5 top-1/2 -translate-y-1/2 text-pink-300 font-black text-xl">¥</span>
            </div>
          </section>
          <button onClick={handleSaveSettings} disabled={isSaving} className={`w-full py-5 rounded-[32px] shadow-xl font-black text-white text-lg ${isSaving ? 'bg-gray-300' : 'bg-gradient-to-r from-pink-400 to-rose-400'}`}>
            {isSaving ? 'Saving...' : '設定を保存する ✨'}
          </button>
        </div>
        <button onClick={async () => { await supabase.auth.signOut(); router.push('/login'); }} className="w-full py-10 text-gray-300 text-xs font-black tracking-[0.3em] uppercase italic">Logout</button>
      </main>
      {/* @ts-ignore */}
