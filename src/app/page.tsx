...
export default function Page() {
  ...
  return (
    <div className="min-h-screen bg-[#FFF9FA] text-gray-800 pb-40 font-sans">
      ...
      <main className="px-3 mt-4 space-y-4">
        ...
        <section className="bg-[#FFE9ED] rounded-[22px] p-4 border border-pink-300 relative overflow-hidden shadow-sm">
          ...
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-3 gap-1 bg-white/80 rounded-xl py-2 border border-pink-200 text-center">
              ...
              <div className="border-x border-pink-100"><p className="text-[10px] text-pink-400 font-bold">初指名</p><p className="text-xl font-black text-pink-600">{monthlyTotals.first}</p></div>
              <div><p className="text-[10px] text-pink-400 font-bold">本指名</p><p className="text-xl font-black text-pink-600">{monthlyTotals.main}</p></div>
            </div>
          </div>
        </section>
        ...
        <section className="bg-white rounded-[24px] border border-pink-300 shadow-xl overflow-hidden">
          <div className="bg-[#FFF5F6] p-3 px-4 flex justify-between items-center border-b border-pink-100">
            <h3 className="text-[18px] font-black text-gray-800">{selectedDate ? format(selectedDate, 'M/d (eee)', { locale: ja }) : ''}</h3>
            <span className="text-pink-500 font-black text-lg">{selectedShift ? `${selectedShift.start_time}~${selectedShift.end_time}` : 'OFF'}</span>
          </div>
          {selectedShift && (
              ...
              <div className="grid grid-cols-3 gap-2">
                {['f', 'first', 'main'].map((key) => (
                  <div key={key} className="text-center space-y-1">
                    <label className="text-[14px] font-black block text-gray-900">{key==='f'?'フリー':key==='first'?'初指名':'本指名'}</label>
                    <input type="number" inputMode="numeric" placeholder="0" value={editReward[key as keyof typeof editReward]} onFocus={e=>e.target.select()} onChange={e=>setEditReward({...editReward,[key]:e.target.value})} className={`w-full text-center py-2 bg-[#FAFAFA] rounded-lg font-black text-2xl border ${editReward[key as keyof typeof editReward]===''?'text-gray-200 border-gray-100':'text-pink-500 border-pink-200'}`} />
                  </div>
                ))}
              </div>
              
              <div className="bg-pink-50/30 p-3 rounded-xl border border-pink-100 flex items-center justify-between h-[64px]">
                <label className="text-[14px] font-black text-gray-900">本日の報酬</label>
                <div className="flex items-center flex-1 justify-end pl-4 font-black">
                  <span className="text-pink-200 text-2xl mr-1">¥</span>
                  <input type="text" inputMode="numeric" placeholder="0" value={editReward.amount!==''?Number(editReward.amount).toLocaleString():''} onFocus={e=>e.target.select()} onChange={e=>{const v=e.target.value.replace(/,/g,''); if(/^\d*$/.test(v))setEditReward({...editReward,amount:v});}} className={`w-full text-right bg-transparent text-[32px] focus:ring-0 border-none ${editReward.amount===''?'text-gray-200':'text-pink-500'}`} />
                </div>
              </div>

              <button onClick={handleSaveReward} className="w-full bg-pink-500 text-white font-black py-5 rounded-xl text-2xl shadow-lg active:scale-95 transition-all">実績を保存 💾</button>
            </div>
          )}
        </section>
        ...
        <p className="text-center text-[10px] font-bold text-gray-200 tracking-widest pb-8">ver 1.16.6</p>
      </main>
      ...
      <footer className="fixed bottom-0 left-0 right-0 z-[9999] bg-white/95 backdrop-blur-md border-t border-pink-100 pb-6 pt-3 shadow-sm">
        <nav className="flex justify-around items-center max-w-sm mx-auto px-4 text-gray-300">
          <button className="flex flex-col items-center text-pink-500" onClick={() => router.push('/')}><span className="text-xl">🏠</span><span className="text-[9px] font-black uppercase">Home</span></button>
          <button className="flex flex-col items-center" onClick={() => router.push('/salary')}><span className="text-xl">💰</span><span className="text-[9px] font-black uppercase">Salary</span></button>
          <button onClick={() => supabase.auth.signOut().then(() => router.push('/login'))} className="flex flex-col items-center"><span className="text-xl">🚪</span><span className="text-[9px] font-black uppercase">Logout</span></button>
        </nav>
      </footer>
    </div>
  );
