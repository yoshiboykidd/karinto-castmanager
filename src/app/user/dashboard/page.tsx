...
function DashboardContent() {
  ...
  useEffect(() => {
    ...
    // 2. 📍 マイページの設定を安全に読み込む（ここでタブの並びと表示が決まる）
    let targetStores = defaultStores;
    try {
      const savedV2 = localStorage.getItem('user_favorite_shops_v2');
      if (savedV2) {
        const parsed = JSON.parse(savedV2);
        // visibleが明示的にfalseでないものを、保存された順番で抽出
        const visibleOnly = parsed.filter((s: any) => s.visible !== false).map((s: any) => s.name);
        if (visibleOnly.length > 0) {
          targetStores = visibleOnly;
        }
      }
    } catch (e) {
    ...
    setActiveStore(targetStores[0]); // 一番左のタブを初期選択

    // 3. データ取得（前回「表示された」と確認済みの最も安全なロジック）
