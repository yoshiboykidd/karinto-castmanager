...
function DashboardContent() {
  ...
  useEffect(() => {
    ...
    setUserName(JSON.parse(sessionData).name || 'お客様');

    // 📍 お気に入り順序と非表示設定を読み込み
    const savedV2 = localStorage.getItem('user_favorite_shops_v2');
    let targetStores: string[] = [];
    if (savedV2) {
      targetStores = JSON.parse(savedV2).filter((s: any) => s.visible === true).map((s: any) => s.name);
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24 text-slate-800 font-sans">
      <header className="bg-white px-6 py-4 flex justify-between items-center sticky top-0 z-10 shadow-sm border-b border-slate-100">
      ...
      <main className="px-6 pt-6 space-y-6">
        ...
        <section className="space-y-3">
          {loading ? <div className="text-center py-20 text-slate-300 font-black text-[10px] uppercase tracking-widest">Loading...</div> : 
            filteredShifts.length > 0 ? filteredShifts.map((shift, idx) => (
              <div key={idx} className="bg-white rounded-[28px] p-4 flex items-center space-x-4 border border-slate-100 shadow-sm transition-all active:scale-[0.98]">
                ...
                <div className="flex-1">
                  ...
                  <h4 className="text-lg font-black text-slate-800 mt-1 leading-none tracking-tight">{shift.hp_display_name}</h4>
                  <div className="flex items-center text-slate-400 mt-3 text-sm font-black italic tracking-tighter">
                    <Clock size={14} className="mr-1 text-blue-300" /> {shift.start_time} — {shift.end_time}
                  </div>
                </div>
              </div>
            )) : <div className="text-center py-20 bg-white rounded-[32px] border border-dashed border-slate-200 text-slate-300 font-bold text-sm tracking-tight">本日の出勤データはありません</div>
          }
        </section>
      </main>
      <nav className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-xl border-t border-slate-100 px-8 py-4 flex justify-between items-center z-20 shadow-lg">
        <button onClick={() => router.push('/user/dashboard')} className="flex flex-col items-center text-blue-500"><Home size={24} /><span className="text-[10px] font-black mt-1 uppercase">Home</span></button>
        ...
        <button onClick={() => router.push('/user/profile')} className="flex flex-col items-center text-slate-300"><User size={24} /><span className="text-[10px] font-black mt-1 uppercase">Mypage</span></button>
      </nav>
    </div>
  );
}
