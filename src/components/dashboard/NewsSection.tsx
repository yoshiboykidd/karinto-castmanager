'use client';

import { useState } from 'react';
import { format, parseISO } from 'date-fns';

// 📍 修正：target を型定義に追加
type NewsItem = {
  id: string;
  created_at: string;
  title?: string;
  body?: string;
  image_url?: string;
  content: string;
  target?: string; // 📍 追加：'cast' | 'user'
};

type NewsSectionProps = {
  newsList: NewsItem[];
};

export default function NewsSection({ newsList }: NewsSectionProps) {
  const [expandedId, setExpandedId] = useState<string | null>(null);

  // 📍 修正：キャスト向けのお知らせだけを抽出（念のためのガードロジック）
  // targetが未設定（古いデータ）または 'cast' のものを表示します
  const castNews = newsList.filter(n => n.target === 'cast' || !n.target);

  return (
    <section className="bg-white rounded-[28px] border border-gray-100 shadow-sm overflow-hidden mb-8">
      <div className="bg-gray-50 p-2.5 px-6 text-[11px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100 flex justify-between items-center">
        <span>Cast News</span>
        <span className="text-[9px] bg-pink-100 text-pink-500 px-2 py-0.5 rounded-full">運営からのお知らせ</span>
      </div>
      
      <div className="divide-y divide-gray-50">
        {castNews.length === 0 ? (
          <div className="p-8 text-center text-gray-300 text-xs font-bold italic">
            新しいお知らせはありません
          </div>
        ) : (
          castNews.map((n) => (
            <div 
              key={n.id} 
              className="p-4 px-6 cursor-pointer active:bg-gray-50 transition-colors"
              onClick={() => setExpandedId(expandedId === n.id ? null : n.id)}
            >
              <div className="flex gap-4 items-start">
                <span className="text-[10px] text-pink-400 font-black shrink-0 bg-pink-50 px-2 py-1 rounded leading-none mt-0.5">
                  {format(parseISO(n.created_at), 'MM/dd')}
                </span>
                <div className="flex-1">
                  <p className="text-[13px] font-black text-gray-700 leading-relaxed">
                    {n.title || n.content}
                  </p>

                  {expandedId === n.id && (n.body || n.image_url) && (
                    <div className="mt-4 space-y-3 animate-in fade-in slide-in-from-top-1">
                      {n.image_url && (
                        <img 
                          src={n.image_url} 
                          alt="news" 
                          className="w-full h-auto rounded-2xl border border-gray-100 shadow-sm"
                        />
                      )}
                      {n.body && (
                        <p className="text-[12px] font-medium text-gray-500 leading-relaxed whitespace-pre-wrap">
                          {n.body}
                        </p>
                      )}
                    </div>
                  )}
                </div>
                <span className={`text-[10px] text-gray-300 transition-transform ${expandedId === n.id ? 'rotate-180' : ''}`}>
                  ▼
                </span>
              </div>
            </div>
          ))
        )}
      </div>
    </section>
  );
}