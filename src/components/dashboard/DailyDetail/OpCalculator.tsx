...
export default function OpCalculator({ selectedRes, initialTotal, supabase, onToast, onClose, isInCall, setIsInCall }: any) {
  ...
  const toggleOp = (no: string, text: string, price: number) => {
    setSelectedOps((prev) => {
      ...
      if (isAlreadySelected) return prev.filter(op => op.no !== no);
  ...
  const sendNotification = async (type: 'START' | 'ADD' | 'HELP') => {
    ...
    if (type === 'HELP') {
      message = `【呼び出し】${selectedRes.customer_name}様のお部屋：キャストが呼んでいます！`;
    } else if (type === 'START') {
      message = `【入室】${selectedRes.customer_name}様：¥${displayTotal.toLocaleString()}（内訳:${opNames || 'OPなし'}）`;
    } else {
      message = `【追加】${selectedRes.customer_name}様：追加OP（${opNames}）計¥${opsTotal.toLocaleString()}`;
    }
    ...
    try {
      ...
      setSelectedOps([]); 
      onToast(type === 'HELP' ? "スタッフを呼びました" : "店舗へ通知しました");
    ...
    <div className="fixed inset-0 z-[300] flex flex-col bg-gray-900 text-white animate-in fade-in duration-200 overflow-hidden">
      ...
      <div className="bg-gray-800 border-b border-gray-700 px-3 py-2.5 min-h-[54px] flex flex-wrap gap-1.5 shrink-0 items-center overflow-y-auto max-h-[140px]">
        {selectedOps.length === 0 ? (
          <p className="text-[11px] text-gray-500 font-black italic opacity-60">※ オプションを選択してください</p>
        ) : (
          selectedOps.map((op) => (
            <button key={op.no} onClick={() => toggleOp(op.no, op.name, op.price)}
              className="bg-pink-600 border border-pink-400 text-white px-2 py-1 rounded-lg text-[11px] font-black flex items-center gap-1 active:scale-95 transition-all">
          ...
          <div key={cat.label} className="space-y-2">
            ...
            <div className="grid grid-cols-5 gap-1.5">
              {cat.items.map((item) => {
                ...
                return (
                  <button key={item.n} onClick={() => toggleOp(item.n, item.t, cat.price)}
      ...
      {/* 📍 通知ボタンエリア */}
      <div className="p-4 bg-gray-900/95 backdrop-blur-xl border-t border-gray-800 fixed bottom-0 left-0 right-0 z-40 flex flex-col gap-2">
        <div className="flex gap-2">
          {/* ヘルプボタン（スタッフ呼び出し） */}
          <button 
            onClick={() => sendNotification('HELP')}
            disabled={isSending}
            className="flex-1 py-3 bg-gray-700 text-white rounded-xl font-black text-[14px] active:scale-95 transition-all"
          >
            ✋ スタッフ呼出
          </button>
