    ...
    <div className="fixed inset-0 z-[100] flex items-start justify-center p-2 overflow-y-auto bg-black/90 backdrop-blur-sm pt-4 pb-24 text-gray-800">
      ...
      <div className="relative bg-white w-full max-w-[340px] rounded-[32px] shadow-2xl animate-in zoom-in duration-150 flex flex-col">
        <div className="p-2 px-4 flex items-center justify-center gap-3 relative border-b border-gray-50">
          <button onClick={onClose} className="absolute top-2 right-3 text-gray-300"><X size={20} /></button>
        ...
        <div className="px-4 py-4 space-y-5">
            ...
            {isEditingMemo ? (
              <div className="p-3 space-y-2 animate-in slide-in-from-top-2">
                {!selectedRes.cast_memo && customerInfo.latestMemo && (
                  <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white/60 rounded-xl border border-pink-100 text-[10px] font-black text-pink-400 italic"><MessageSquare size={12} /> 前回のメモを引き継いでいます</div>
                )}
                <textarea className="w-full p-4 rounded-2xl border-2 border-pink-200 bg-white text-[16px] font-bold focus:outline-none focus:border-pink-400 min-h-[160px] shadow-inner" value={memoDraft} onChange={(e) => setMemoDraft(e.target.value)} placeholder="特徴、NG、会話内容など..." autoFocus />
                <button onClick={onSaveMemo} className="w-full h-14 bg-pink-500 text-white rounded-2xl flex items-center justify-center gap-2 font-black text-[15px] shadow-lg shadow-pink-200"><Save size={18} /> 保存して閉じる</button>
              </div>
            ) : (
          ...
          <div className="space-y-2 pt-2">
            ...
            <button onClick={onDelete} disabled={isDeleting} className="w-full h-10 rounded-xl text-gray-300 flex items-center justify-center gap-1 font-bold text-[11px] disabled:opacity-50">{isDeleting ? <Loader2 size={13} className="animate-spin" /> : <Trash2 size={13} />}予約を取り消す</button>
          </div>
        </div>
      </div>
    </div>
