...
export default function ReservationModal({ 
  ...
  const [isOpOpen, setIsOpOpen] = useState(false); // 📍 別窓の開閉管理
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-1">
      ...
      <div className="relative w-full max-w-sm bg-white rounded-[24px] flex flex-col max-h-[98vh] overflow-hidden text-gray-800 shadow-2xl">
        <div className="px-4 py-2 border-b border-gray-100 flex justify-between items-center shrink-0">
          <div className="flex items-center gap-2"><p className="text-[18px] font-black">{String(selectedRes.reservation_date || "").replace(/-/g, '/')}</p></div>
        ...
        <div className="overflow-y-auto px-2 pt-2 pb-12 space-y-1.5 flex-1 overscroll-contain">
          ...
          <button 
            ...
            className="w-full bg-gray-900 rounded-[18px] p-4 text-left shadow-lg active:scale-[0.98] transition-all relative overflow-hidden group"
          >
            <div className="flex justify-between items-end">
              ...
              <div className="bg-white/10 px-3 py-2 rounded-xl text-[12px] font-black text-white group-hover:bg-white/20">
                OP計算・開始 🚀
              </div>
            </div>
          </button>
            ...
            {isEditingMemo ? (
              <div className="p-2 space-y-1.5">
                <textarea
                  ...
                  className="w-full h-24 p-2 bg-white rounded-xl text-[16px] font-bold focus:outline-none resize-none border-none shadow-inner"
                  autoFocus
                />
                <div className="flex gap-1">
          ...
          <button onClick={() => onDelete?.()} className="w-full py-2 text-gray-300 font-bold text-[10px] mt-1">
