...
import { Clock, X, MapPin, Tag, Home, History, CreditCard, User } from 'lucide-react';
...
export default function DailyDetail({ date, dayNum, shift, reservations = [], theme = 'pink' }: any) {
  ...
  return (
    <>
      <section className="relative overflow-hidden rounded-[32px] border bg-white border-pink-100 shadow-xl p-3 pt-10 flex flex-col space-y-3 subpixel-antialiased">
        
        {(isKarin || isSoine) && (
          <div className={`absolute top-0 left-0 right-0 py-2 text-center font-black text-[14px] tracking-[0.4em] z-20 text-white shadow-md [text-shadow:_1px_1px_0_rgba(0,0,0,0.2)]
        ...
        <div className="flex items-center justify-between px-1 mt-1 relative h-9">
          {/* 日付：数字24px / 記号12px */}
          <div className="flex items-baseline font-black text-gray-800 tracking-tighter [text-shadow:_0.4px_0_0_currentColor] shrink-0 z-10">
            <span className="text-[24px] leading-none">{format(date, 'M')}</span>
            <span className="text-[12px] opacity-30 mx-0.5 font-bold">/</span>
            <span className="text-[24px] leading-none">{format(date, 'd')}</span>
            <span className="text-[12px] opacity-30 ml-0.5 font-bold">({format(date, 'E', { locale: ja })})</span>
          </div>

          {/* シフト時間：カードの完全中央に配置 / 数字24px */}
          <div className="absolute left-1/2 -translate-x-1/2 flex items-center gap-1.5 whitespace-nowrap z-10">
            {isOfficial ? (
              <>
                ...
                <div className={`flex items-baseline font-black tracking-tighter ${accentColor} [text-shadow:_0.6px_0_0_currentColor]`}>
                  ...
                  <span className="text-[12px] mx-1 opacity-20 font-bold">〜</span>
                  <span className="text-[24px] leading-none">{shift?.end_time}</span>
                </div>
              </>
            ) : (
              <span className="text-[12px] font-black text-gray-200 italic uppercase tracking-widest leading-none">Day Off</span>
            )}
          </div>
      ...
      {selectedRes && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-[340px] rounded-[28px] p-4 shadow-2xl flex flex-col space-y-2.5 subpixel-antialiased text-gray-800">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="bg-gray-50 p-2.5 rounded-2xl border border-gray-100 flex flex-col justify-center">
                ...
                <p className="text-[9px] font-black text-gray-300 uppercase tracking-tighter leading-none mt-0.5">No.{selectedRes.customer_no}</p>
              </div>
              <div className="bg-gray-50 p-2.5 rounded-2xl border border-gray-100 flex flex-col items-center justify-center">
                <div className="flex items-baseline justify-center font-black text-gray-800 tracking-tighter">
                  <span className="text-lg">{selectedRes.visit_count}</span>
