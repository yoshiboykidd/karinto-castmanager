  ...
  return (
    <div className="w-full">
      <div className="flex items-center justify-between mb-4 px-4 font-black text-slate-700">
        <button onClick={() => onMonthChange(new Date(month.getFullYear(), month.getMonth() - 1, 1))}><ChevronLeft /></button>
        <span className="text-lg tracking-tighter">{format(month, 'yyyy / M月')}</span>
        <button onClick={() => onMonthChange(new Date(month.getFullYear(), month.getMonth() + 1, 1))}><ChevronRight /></button>
      </div>
        ...
        {days.map(day => {
          ...
          return (
            <div key={dateStr} onClick={() => onSelect(day)} 
              className={`relative h-12 w-full flex flex-col items-center justify-center rounded-2xl transition-all active:scale-95
              ...
              {isRequested && !isOfficial && <div className="absolute inset-1 rounded-full border-2 border-purple-300 border-dashed animate-pulse" />}
              {isSpecial && <div className="absolute top-1 right-1 h-1.5 w-1.5 rounded-full bg-yellow-400 shadow-sm" />}
            </div>
          );
        })}
      </div>
